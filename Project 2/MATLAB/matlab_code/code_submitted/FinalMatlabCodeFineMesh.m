...
if strcmpi(plot_flux,'yes')
    ...
    axis equal tight;
    title('Velocity Field-Fine Mesh');
    xlabel('X'); ylabel('Y');
end
...
title('Fine Mesh with Boundary Conditions');
xlabel('X'); ylabel('Y');
...
axis equal tight;
colorbar;
title('Pressure Field (Bernoulli)-Fine Mesh');
xlabel('X'); ylabel('Y');
...
fprintf('\nAverage Half Band Width (AHBW) for fine Mesh = %.2f\n',AHBW);
